...
// #define USE_INTERNAL_IMU    // use internal gyro (internal I2C)
// #define USE_EXTERNAL_IMU    // use unit gyro (external I2C)
// #define USE_FACES              // use FACES (GAME / ENCODER / JOYSTICK)
// #define USE_UNIT_ENCODER    // UNIT ENCODER (external I2C)
// #define USE_UNIT_JOYSTICK   // UNIT JOYSTICK (external I2C)
// #define USE_ANALOG_G36      // UNIT ANGLE / UNIT FADER ( GPIO36 analog read )
// #define USE_EXTERNAL_SPK    // use spk_hat or atomic_spk
...
// for ATOM Display setting
#define M5ATOMDISPLAY_LOGICAL_WIDTH 320
#define M5ATOMDISPLAY_LOGICAL_HEIGHT 180
#define M5ATOMDISPLAY_REFRESH_RATE 60.0f

/*/ /// for High Refresh Rate Display
#define M5ATOMDISPLAY_LOGICAL_WIDTH 256
#define M5ATOMDISPLAY_LOGICAL_HEIGHT 144
#define M5ATOMDISPLAY_REFRESH_RATE 125.0f
//*/

// #include <M5AtomDisplay.h>   // use ATOM Display
...
#if defined ( USE_ANALOG_G36 )
  {
    target_lr += (ANALOG_CENTER - analogRead(36)) * 10;
// Serial.printf("analog:%d\n", analogRead(36));
...
#if defined ( USE_UNIT_JOYSTICK )

  if (M5.Ex_I2C.start(0x52, true, 400000))
  {
    uint8_t buf[3];
    if (M5.Ex_I2C.read(buf, 3))
    {
      target_lr += (int8_t)(buf[0] - 0x7F) * 128;
      input_pressed |= buf[2];
      //Serial.printf("e:%02x %02x %02x\r\n", buf[0], buf[1], buf[2]);
    }
    M5.Ex_I2C.stop();
  }
...
#if defined ( USE_HAT_JOYSTICK )

  uint8_t buf[3];
  if (M5.Ex_I2C.readRegister(0x38, 0x02, buf, 3, 400000))
  {
    if (buf[0] != 0xFF)
    {
      target_lr += ((int8_t)buf[1]) * 160;
      input_pressed |= !buf[2];
    }
    // Serial.printf("e:%02x %02x %02x\r\n", buf[0], buf[1], buf[2]);
  }
  ...
  if (M5.In_I2C.start(0x5E, true, 400000))
  {
    uint8_t buf[5];
    if (M5.In_I2C.read(buf, 5))
    {
      if (buf[2] == 0xFF && buf[3] == 0xFF && buf[4] == 0xFF) // FACES ENCODER
      {
        static int prev_value = INT16_MAX;
        if (prev_value == INT16_MAX)
        {
          prev_value = 0;
        }
        else
        {
          prev_value += (int8_t)buf[0];
        }
        input_pressed |= !buf[1];
        target_lr += prev_value * 2048;
      }
      else  // FACES JOYSTICK
      {
        target_lr += ((int)(buf[3] << 8 | buf[2]) - 0x200) * 48;
        input_pressed |= !buf[4];
      }
//  Serial.printf("b:%d  %02x %02x %02x %02x %02x\r\n", target_lr, buf[0], buf[1], buf[2], buf[3], buf[4]);
    }
    M5.In_I2C.stop();
  }

  if (M5.In_I2C.start(0x08, true, 400000))
  {
    uint8_t buf[1];
    if (M5.In_I2C.read(buf, 1))
    {
//  Serial.printf("b:%d  %02x %02x %02x %02x %02x\r\n", target_lr, buf[0], buf[1], buf[2], buf[3], buf[4]);
      {
        static int prev_value = 0;
...
#if defined ( USE_UNIT_ENCODER )

  {
    uint8_t buf[2];
    if (M5.Ex_I2C.readRegister(0x40, 0x10, buf, 2, 400000))
    {
      static int32_t initial_value = INT32_MAX;
      int value = (int16_t)(buf[1] << 8 | buf[0]);
      if (initial_value == INT32_MAX)
      {
        initial_value = value;
      }
      else
      {
        target_lr += (value - initial_value) * 1024;
      }
    }
  }
...
#if defined ( USE_INTERNAL_IMU ) || defined ( USE_EXTERNAL_IMU )
    {
      float x, y, z;
      bool res = M5.Imu.getAccel(&x, &y, &z);
      // Serial.printf("res:%d x:%f y:%f z:%f\n", res, x, y, z);
      if (res)
      {
        switch (M5.getBoard())
        {
...
int32_t funcInputLeftRight(void)
{
  ...
  {
    return (x > display.width() >> 1) ? 16384 : -16384;
  }
  return 0;
}
...
//static LGFX display(160, 80, 4);
//static LGFX display(240, 135, 4);
//static LGFX display(320, 240, 4);
static LGFX display(1280, 720, 1);
//static LGFX display(1920, 1080, 1);
//static LGFX display2(1024, 1024, 1);

#include <thread>

static constexpr size_t PRO_CPU_NUM = 0;

void xTaskCreatePinnedToCore(void(*func)(void*)
  , const char*
  , size_t
  , void* args
  , size_t
...
int32_t funcInputLeftRight(void)
{
  ...
  if (display.getTouch(&x, &y))
  {
    return ((x - (display.width() >> 1)) << 16) / display.width();
  }

  if (!lgfx::gpio_in(39)) { target_lr -= 16384; }
  if (!lgfx::gpio_in(37)) { target_lr += 16384; }
  int add = (target_lr == 0) ? 192 : 384;

  if (current_lr != target_lr)
  {
    if (current_lr < target_lr) { current_lr = std::min(target_lr, current_lr + add); }
    else { current_lr = std::max(target_lr, current_lr - add); }
  }
  return current_lr;
}
  ...
  for (;;)
  {
    ...
    if (game_mode == game_mode_t::stage_select)
    {
      ...
      else
      {
        if (enter)
        {
          ...
          auto t = std::time(nullptr) + 5; //start after 5 sec.
        ...
        /*
                if (player_steering_angle > 8192)
                {
                  stage_index_x256 = stage_index << 8;
                  stage_index_x256_target = (++stage_index << 8);
                  if (stage_index >= stage_data_max)
                  {
                    stage_index = 0;
                  }
                }
                else if (player_steering_angle < -8192)
                {
                  if (stage_index == 0)
                  {
                    stage_index = stage_data_max;
                  }
                  stage_index_x256 = stage_index << 8;
                  stage_index_x256_target = (--stage_index << 8);
                }
    ...
    if (msec_lapstart)
    {
      ...
      if (lap_msec >= 5000)
      {
        ...
        lap_sec -= lap_min * 60;
        //*
        snprintf(lap_str, sizeof(lap_str), "%02d:%02d:%02d", lap_min, lap_sec, lap_msec);
        /*/
        ...
        sprintf(lap_str, "%02d:%02d:%02d fps%d", lap_min, lap_sec, lap_msec, fps);
        //*/
      }
    }
...
void setup(void)
{
  ...
  // Increase heap (may not make sense in some environments)
...
#endif
#if defined ( USE_EXTERNAL_IMU )
  cfg.external_imu = true;
#endif
#if defined ( USE_EXTERNAL_SPK )
  ...
  M5.Ex_I2C.begin(I2C_NUM_0, GPIO_NUM_0, GPIO_NUM_26);

#endif

#if defined ( USE_UNIT_ENCODER ) || defined ( USE_UNIT_JOYSTICK )
  ...
  M5_LOGI("1heap:%u",esp_get_free_heap_size());
  configTime(); // NTP 
  M5_LOGI("2heap:%u",esp_get_free_heap_size());

  M5_LOGW(">wifi");
  WiFi.mode(WIFI_STA);
  if(esp_now_init() != ESP_OK)
  {
    M5_LOGE("Failed to initialize ESP-NOW");
    display.clear(TFT_RED);
  }
  M5_LOGW("<<wifi");
  ...
  M5_LOGI("p0heap:%u",esp_get_free_heap_size());
  setupPingPongBuffer(&display);
  M5_LOGI("p1heap:%u",esp_get_free_heap_size());
  ...
  auto ret = xTaskCreatePinnedToCore(gameTask, "gameTask", 1024 * 3, nullptr, 2, nullptr, PRO_CPU_NUM);
  M5_LOGI("create task:%d", ret);
#else
  SDL_CreateThread((SDL_ThreadFunction)gameTask, "gameTask", nullptr);
#endif
  //game_mode = game_mode_t::stage_select;
