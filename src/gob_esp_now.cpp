...
void Communicator::update()
{
    ...
    {
        // Remove acked data
        if(!remove_acked(q.first, q.second)) { /*_state[q.first].reset();*/  continue; }
        // Transmission is not possible before receiving the transmission callback
        if(!_canSend){ continue; }

        // LIB_LOGD("  >[%s] st:%d", q.first.toString().c_str(), _state[q.first].state);
        // Send / resend
            ...
            ms > _state[q.first].sentTime + _config.retransmissionTimeout))
        {
            ...
            if(!q.first)
            {
                for(auto& ss : _state)
                {
                    // TODO: 単純代入で良いのか? StateもAll版必要?
                    if(ss.second.state == State::Failed) { ss.second.retry = _state[q.first].retry; }
                };
            }
            #endif
        }
    ...
    if(!_transceivers.empty() && _canSend)
    {
        for(auto& ss : _state)
        {
            ...
            {
                LIB_LOGD("[NUL]");
                // TODO: 適当なトランシーバーでは post all peer していると seq ずれる...
                // nul は commnicator レベルで処理しないとダメ!!
...
bool Communicator::remove_acked(const MACAddress& addr, std::vector<uint8_t>& packet)
{
    ...
    while(cnt--)
    {
        ...
        if(th->isACK())
        {
            ...
            {
                return th->hasPayload() && t->delivered(th->rudp.sequence, addr);
            });
    ...
    if(ch->count != oldCount) { LIB_LOGD("[%s] CH:%u -> %u",
                                         addr.toString().c_str(), oldCount, ch->count); }
    if(ch->count == 0) { packet.clear(); }
            ...
            if(t->identifier() == th->tid)
            {
                ...
                auto correct = !th->isACK() ||
                        t->with_lock([&t, &ch, &th, &addr]()
                        {
                            ...
                            auto rseq = restore_u64_later(
                                th->hasPayload() ? t->_peerInfo[addr].recvSeq : t->_peerInfo[addr].recvAckSeq,
                                th->rudp.sequence);
                            // Correct order?
                            return rseq ==
                                    ((th->hasPayload() ? t->_peerInfo[addr].recvSeq : t->_peerInfo[addr].recvAckSeq) + 1);
                        });
...
void Transceiver::on_receive(const MACAddress& addr, const TransceiverHeader* th)
{
    ...
    if(th->isACK())
    {
        ...
        s = restore_u64_later(pi.recvSeq, th->rudp.sequence);
        (th->hasPayload() ?  pi.recvSeq : pi.recvAckSeq) = s;
        a = restore_u64_later(pi.recvAck, th->rudp.ack);
        if(a > pi.recvAck) { pi.recvAck = a; }
        pi.recvTime = millis();
        pi.needReturnACK |= (th->hasPayload() || th->isNUL());
    }
    LIB_LOGD("[RECV]:%u 0x%02x P:%u RS:%llu RA:%llu => %llu:%llu",
             th->tid, th->rudp.flag, th->payloadSize(), s, a, pi.recvSeq, pi.recvAck);
    if(th->isRST())
    {
        LIB_LOGD("[RECV]:RST");
